...
```
@birderrr/fbrpc-core     — ApiDef / ApiCall / StreamCall / RpcError / AnyApiHandler（零依赖）
@birderrr/fbrpc-server   — createRouter() + scanModules()（Fastify 插件）
@birderrr/fbrpc-client   — createClient<T>() Proxy 客户端 + streamRequest()
```
...
```ts
import type { AnyApiHandler, ApiCall } from "@birderrr/fbrpc-core";
