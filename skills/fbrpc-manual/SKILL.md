...
// satisfies ServiceHandlers<AuthProtocol> 做三件事：
//   1. 编译期检查所有协议方法都已实现
//   2. 禁止多余方法
//   3. 每个 handler 的 call.req / call.res 自动窄化——不用手写 C<K>
export const handlers = {
  ...
  async register(call: ApiCall<AuthProtocol["register"]>) {
    // ...
  },
...
- **全量使用 `satisfies ServiceHandlers<Protocol>`** — 编译期强制全覆盖，协议加方法立刻报错
- 每个 handler 的 call 标注 `ApiCall<Protocol["method"]>` — `satisfies` 自动窄化，不用手写类型缩写
- 不用 `Record<string, AnyApiHandler>` — 它丢掉了 key 约束，协议遗漏不报错
