...
```
@aiwriter/fbrpc-core     — ApiDef / ApiCall / StreamCall / RpcError / AnyApiHandler（零依赖）
@aiwriter/fbrpc-server   — createRouter() + scanModules()（Fastify 插件）
@aiwriter/fbrpc-client   — createClient<T>() Proxy 客户端 + streamRequest()
```
...
```ts
import type { ApiDef } from "@aiwriter/fbrpc-core";
...
```ts
import type { AnyApiHandler, ApiCall } from "@aiwriter/fbrpc-core";
...
```ts
import { createRouter } from "@aiwriter/fbrpc-server";
...
```ts
import { createClient } from "@aiwriter/fbrpc-client";
